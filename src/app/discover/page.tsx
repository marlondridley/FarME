...
export default function DiscoverProducePage() {
  ...
  const [result, setResult] = useState<DiscoverProduceOutput | null>(null);
  ...
  const onSubmit: SubmitHandler<DiscoverProduceInput> = async (data) => {
    setLoading(true);
    setResult(null);
    try {
      ...
      {result && (
        <div className="space-y-6 animate-in fade-in-50 mt-8">
          <Card className="bg-primary/5 border-primary/20">
            ...
            <CardContent>
              <p className="whitespace-pre-wrap font-medium">{result.suggestedProducts}</p>
            </CardContent>
          </Card>
